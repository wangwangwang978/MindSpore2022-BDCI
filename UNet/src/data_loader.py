...
class MyMultiClassDataset:
    ...
    def __init__(self, data_dir, repeat, is_train=False, split=0.8, shuffle=False):
        ...
        self.is_train = is_train
        self.split = split
        assert split >=0 and split <=1.0, 'split value error!'
        print('foreground: ', len(glob.glob(os.path.join(data_dir,'foreground','*.npz'))))
        print('background: ', len(glob.glob(os.path.join(data_dir,'background','*.npz'))))
        self.img_ids = sorted(glob.glob(os.path.join(data_dir,'foreground','*.npz')) + glob.glob(os.path.join(data_dir,'background','*.npz')) )
    ...
    def _read_img_mask(self, img_npz):
        data = np.load(img_npz)
...
class MyPredDataset:
    ...
    def __init__(self, data_dir):
        self.data_dir = data_dir
        self.img_ids = sorted(glob.glob(os.path.join(data_dir,'case_*')))[210:]
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.img_ids[index], 'imaging.nii.gz')
...
def scaleIntensityRange(data, src_min=-500, src_max=1000, tgt_min=0.0, tgt_max=1.0, is_clip=True):
    """
    ...
    if src_max - src_min == 0.0:
        ...
        return data - src_min + tgt_min
    data = (data - src_min) / (src_max - src_min)
    data = data * (tgt_max - tgt_min) + tgt_min
    if is_clip:
        data = np.clip(data, tgt_min, tgt_max)
    return data

def my_preprocess_img_mask(img, mask, num_classes, img_size, augment=False, eval_resize=False):
    """
    Preprocess for multi-class dataset.
    Random crop and flip images and masks when augment is True.
    """
    ...
    mask = (np.arange(num_classes) == mask[..., None]).astype(int)
    mask = mask.transpose(2, 0, 1).astype(np.float32)
    img = img[None,...].astype(np.float32) # 扩充一个维度 不支持float64
    # print('type ', img.dtype, mask.dtype)
...
def create_multi_class_dataset(data_dir, img_size, repeat, batch_size, num_classes=2, is_train=False, augment=False,
                               eval_resize=False, split=0.8, rank=0, group_size=1, shuffle=True, is_pred=False):
    """
    ...
    mc_dataset = MyMultiClassDataset(data_dir, repeat, is_train, split, shuffle)
    ...
    compose_map_func = (lambda image, mask: my_preprocess_img_mask(image, mask, num_classes, tuple(img_size),
...
def create_pred_dataset(data_dir, img_size, repeat, batch_size, num_classes=2, is_train=False, augment=False,
    ...
    cores = multiprocessing.cpu_count()
    num_parallel_workers = min(4, cores // group_size)
    mc_dataset = MyPredDataset(data_dir)
    dataset = ds.GeneratorDataset(mc_dataset, mc_dataset.column_names, shuffle=True,
                                  num_shards=group_size, shard_id=rank,
                                  num_parallel_workers=num_parallel_workers, python_multiprocessing=is_train)
    compose_map_func = (lambda image, mask: my_preprocess_img_mask(image, mask, num_classes, tuple(img_size),
