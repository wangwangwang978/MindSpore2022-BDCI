...
class SegmentationNetwork(nn.Cell):
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    if covert_Ascend310_file:
                        ...
                        bboxes_zeors = np.zeros(shape=(2, 1, 96, 160, 160), dtype=np.float32)  
                        # Task04 3D shape 9, 1, 40, 56, 40 # task01 shape 2, 1, 128, 128, 128
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, mindspore.Tensor], mirror_axes: tuple,
        ...
        x = x.asnumpy()
        input_tensor = np.repeat(x, 2, 0) ##### task04??? 9 2 
