...
class ConvDropoutNormNonlin(nn.SequentialCell):
    ...
    def __init__(self, input_channels, output_channels,
        ...
        if self.dropout_op is not None and self.dropout_op_kwargs['p'] is not None and self.dropout_op_kwargs['p'] > 0:
            ...
            self.dropout = None
        self.instancenorm = self.norm_op(output_channels*2, **self.norm_op_kwargs)  # for task04 * 9  for task01 * 2 和 取消 norm reshape
        self.lrelu = self.nonlin(**self.nonlin_kwargs)
        # print('output_channels*2:', output_channels*2)
    ...
    def construct(self, x):
        ...
        x = self.conv(x)
        n, c, d, w, h = shape(x)
        x = ops.Reshape()(x, (1, n * c, d, h, w)) # 修改了 Reshape
...
class Generic_UNet(SegmentationNetwork):
    ...
    def construct(self, x):
        ...
        x = self.conv_blocks_context[4](x)
        skips.append(x)
        
        x = self.conv_blocks_context[5](x)
        ...
        x = self.tu[3](x)
        x = ops.Concat(1)((x, skips[-(3 + 1)]))
        x = self.conv_blocks_localization[3](x)
        x3 = self.seg_outputs[3](x)
        seg_outputs.append(self.final_nonlin(x3))
        
        x = self.tu[4](x)
        x = ops.Concat(1)((x, skips[-(4 + 1)]))
        x = self.conv_blocks_localization[4](x)
        x4 = self.seg_outputs[4](x)
        seg_outputs.append(self.final_nonlin(x4))
