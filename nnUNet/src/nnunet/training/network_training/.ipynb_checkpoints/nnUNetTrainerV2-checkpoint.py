...
class WithLossCell(nn.Cell):
    ...
    def construct(self, data, y_0, y_1, y_2, y_3, y_4):
        """construct loss"""
        out = self._backbone(data)
        # print('WithLossCell shape:')
        # print(data.shape)
        # for o in out:
        #     print(o.shape)
        # print(y_0.shape, y_1.shape, y_2.shape, y_3.shape, y_4.shape)
        return self._loss_fn(out, y_0, y_1, y_2, y_3, y_4)
...
class WithEvalCell(nn.Cell):
    ...
    def construct(self, data, y0, y1, y2, y_3, y_4):
        """construct forward"""
        outputs = self._network(data)
        loss = self._loss_fn(outputs, y0, y1, y2, y_3, y_4)
        return loss, outputs
...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.max_num_epochs = 1000 # 200
        self.initial_lr = 0.01 # 1e-3
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = nn.Adam(params=self.network.trainable_params(), learning_rate=self.initial_lr, weight_decay=0.0005,loss_scale=1024.0)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=True):
        ...
        target_3 = Tensor(self.one_hot(target[3]), mindspore.float32)
        target_4 = Tensor(self.one_hot(target[4]), mindspore.float32)
        
        # print('data.shape, target_0.shape, target_1.shape, target_2.shape target_3.shape target_4.shape '\
        #      , data.shape, target_0.shape, target_1.shape, target_2.shape, target_3.shape, target_4.shape)

        l = self.train_net(data, target_0, target_1, target_2, target_3, target_4)
        
        if run_online_evaluation:
            output = self.eval_net(data, target_0, target_1, target_2, target_3, target_4)
